...
gps5 <- gps5 %>%
  ...
  mutate(departure_time = lead(arrival_time))%>%
  mutate(time_difference = departure_time - arrival_time)%>%
  filter(time_difference >= 180)
date1 <- as.POSIXct("2014-01-19 00:00:00") #lower bound
date2 <- as.POSIXct("2014-01-19 05:59:59") #upper bound
rng <- interval(date1,date2)         #desired range
gps5 <- gps5[gps5$arrival_time %within% rng,]
