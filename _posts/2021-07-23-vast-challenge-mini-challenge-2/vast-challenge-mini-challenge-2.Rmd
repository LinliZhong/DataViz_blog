...
#gps5<-gps4%>%
#  dplyr::select(id,date,day_of_week,arrival_time,departure_time,lat,long,time_difference)
gps5<-gps4%>%
  relocate(date,.after = id)%>%
  relocate(day_of_week,.after = date)%>%
  relocate(arrival_time, .after= day_of_week)%>%
  relocate(departure_time, .after= arrival_time)%>%
  relocate(lat, .after= departure_time)%>%
  relocate(long, .after= lat)%>%
  relocate(time_difference, .after= long)
DT::datatable(gps4)%>%
    formatRound(columns=c('long', 'lat'), digits=4)
```

We can assume from the above table that latitude with 36.0480 and longitude with 24.8796 is the company's location.
